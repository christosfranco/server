...
struct AuctionEntry
{
    ...
    void SaveToDB() const;
    [[nodiscard]] bool AuctionBidWinning();
    bool QueueWinningBid(PlayerPersistence::InventoryTransaction<Player>& transaction);
    void BlockSaves();

    // -1,0,+1 order result
    int CompareAuctionEntry(uint32 column, const AuctionEntry* auc, Player* viewPlayer) const;

    [[nodiscard]] bool UpdateBid(uint32 newbid, Player* newbidder = NULL); // true only after checked commit, including buyout
};
...
class AuctionHouseObject
{
    public:
        ...
        AuctionEntry* GetAuction(uint32 id) const
        {
            ...
            return itr != AuctionsMap.end() && !itr->second->saveBlocked ? itr->second : NULL;
        }
        ...
        AuctionEntry* AddAuction(PlayerPersistence::InventoryTransaction<Player>& transaction, AuctionHouseEntry const* auctionHouseEntry, Item* newItem, uint32 etime, uint32 bid, uint32 buyout = 0, uint32 deposit = 0, Player* pl = NULL);
...
class AuctionHouseMgr
{
    public:
        ...
        // auction messages
        bool SendAuctionWonMail(AuctionEntry* auction, PlayerPersistence::InventoryTransaction<Player>& transaction);
